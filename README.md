...
Автоматизированная система является огранизационно технической системой,
...
4. Обслуживающий персонал и пользователи АС, объединенный между соблой по
...
В то же время обслуживающий персонал и пользователи часто относят к АС, относя их либо к среде, либо что реже, к вторичным субъектам.
...
**Угроза ИБ** — возможность реализации воздейвствия на информацию, находящуюся в АС, приводящего к блокированию,
...
введения ограничения на круг лиц, имеющих доступ к данной информации и обеспчивается способностью системы сохранять
защищаемую информацию в тайне от лиц, не имеющих доступа к ней.

**Угорза нарушения конфиденциальности** — возможность получения НСД к защищаемой инфомации, находящейся в АС или
...
2. По степени преднамеренности проявления
    ...
    Неправомерное исзменение режима работы устройств или ПО, ввод ошибочных данных, в том числе пересылки данных по ошибочным адресам,
...
3. По непосредственному источнику угроз
    ...
    обязаннастей, так и не для выполнения служебных обязанностей.
    ...
    **Примечание**. пример говорит в частности о том, что должен быть регламент об установке ПО и технические
    средства его реализации и контроля.
        1. Непреднамеренная установа ПО содержащего вирусы и другое злонамеренное ПО.
...
Всё, что выходит за пределы такого пречня — **шпионаж**.
...
позволяющие описать, запомнить, воспроизвести и подвергнуть обработке какие-либо
из указанных категорий или их комплексов.
...
исследования в области компьютерной безопасности. Эти оргинизации называются
_ЦКБ_.
...
2. Уровень средств взаимодействия с носителем
    - Угроза раскр-ия пар-ров: получение инф-ии о ПАС, оприменяемых ср-вах ЗИ, функциях АС
    - Угроза нарушения конф-ти: НСД, перехват сетевого траффика
...
3. Уровень представления
    ...
    - Угроза нарушения целостности: искажение данных (ператор положит ключ в похожий на закрытый контейнер **?** )
    - Угроза отказа служб:
4. уровень содержания информации
    - Угроза раскр-ия пар-ров: Содержание данных на качественном уровне (стркутура отделов по папкам)
    - Угроза нарушения конф-ти:
    - Угроза нарушения целостности: Внедрение дизинформации
...
3. ИБ АС должа обеспечиваться на всех технических этапах обработки информации,
...
6. Защита АС должа предусматирвать контроль эффективности средств защиты.
...
Обучение трудоёмкое (кластер, GPU, ...). Распознавание легко (принимает характиристический вектор).
...
# Раздел 12. Стенаграфические системы защиты
...
**Пример 2**. mp3, там стоит код Хафмана. Раскодируем, добавляем в малозачащие биты сообщение, жмём опять.
...
## Типовая архитектура подсиситемы защиты ОС
...
**Список доступа**. Определяет представление матрицы доступа при котором для каждого объекта определен список переменной длины,
...
## Полномочное разграничение доступа с контролем информационных потокв
